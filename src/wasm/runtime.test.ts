...
import { AmalgamOptions, initRuntime } from "./runtime";
...
describe("Amalgam runtime single threaded", () => {
  let amlg: Awaited<ReturnType<typeof initRuntime>>;

  describe("options", () => {
    describe("logger", () => {
      it("should initialize without logging option", async () => {
    ...
    describe("trace", () => {
      it("should not call logging methods if falsy", async () => {
        const error = jest.fn();
        const warn = jest.fn();
        const info = jest.fn();
        const debug = jest.fn();
        ...
        amlg.verifyEntity("entity.amlg");
        expect(debug).not.toHaveBeenCalled();
      });

      it("should cause method to return trace output through the provided logger.debug method", async () => {
        const error = jest.fn();
        const warn = jest.fn();
        const info = jest.fn();
        const debug = jest.fn();
        ...
        amlg = await getRuntime({ logger, trace: true });
        expect(amlg).toBeInstanceOf(Amalgam);

        amlg.verifyEntity("entity.amlg");
        expect(debug).toHaveBeenLastCalledWith(
          '# RESULT >b\'{"loaded":false,"message":"CAML does not contain a valid header","version":""}\'',
        );
      });

      it("should use the custom logger method of AmalgamTrace if provided", async () => {
        const logger = jest.fn();

        amlg = await getRuntime({ trace: new AmalgamTrace(true, logger) });
        expect(amlg).toBeInstanceOf(Amalgam);
...
const getRuntime = async (options: AmalgamOptions) => {
  const amlg = await initRuntime(options, {
